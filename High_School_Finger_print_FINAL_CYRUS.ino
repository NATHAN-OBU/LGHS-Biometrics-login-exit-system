...
void loop() {
 ...
 // Temp = sensor.getObjectTempCelsius();
  if (!modemConnected) {
    display.printFixed(0,  100, "Connecting to Network", STYLE_BOLD);
    Serial.print(F("Waiting for network..."));
    if (!modem.waitForNetwork()) {
      Serial.println(F(" fail"));
      display.clear();
      display.setColor(GRAY_COLOR4(255));
      display.printFixed(0,  100, "Modem failed", STYLE_BOLD);
    ...
    display.clear();
    display.setColor(GRAY_COLOR4(255));
    display.printFixed(0,  100, "Connected to", STYLE_BOLD);
    //display.print(apn);
  }
  ...
  if (!tb.connected()) {
    ...
    display.clear();
    display.setColor(GRAY_COLOR4(255));
    display.printFixed(0,  100, "Connected to Dash", STYLE_BOLD);
    // display.print( THINGSBOARD_SERVER);
    delay(200);
    display.clear();
    Serial.print(THINGSBOARD_SERVER);
    Serial.print(" with token ");
    //display.printFixed(0,  40, "with token", STYLE_BOLD);
    //display.print(TOKEN);
    ...
    if (!tb.connect(THINGSBOARD_SERVER, TOKEN)) {
      ...
      display.clear();
      display.setColor(GRAY_COLOR4(255));
      display.printFixed(0,  100, "Failed to connect", STYLE_BOLD);
      delay(200);
      display.clear();
      return;
    }
...
void try_again() {
  ...
  // delay(3000); play wih i if it's fast
  ...
  display.setColor(GRAY_COLOR4(128));
  display.drawBitmap1(30, 40, 70, 75, Denied);
  delay(500); //Play with this delay.
}
void accessDenied() {
  display.setFixedFont(ssd1306xled_font8x16);
  display.clear();
